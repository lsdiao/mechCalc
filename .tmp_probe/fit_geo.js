// 验证 timingbelt1 / timingbeltLenChange 几何公式（与探针数据逐字段对比）
var PI = Math.PI;
var PB = { MXL: 2.032, XXL: 3.175, XL: 5.08, L: 9.525, H: 12.7, XH: 22.225, XXH: 31.75, T5: 5 };

// [z1,z2,a0,beltSize] -> {a,zM,zB,beltLen,calBeltLen,kZ,alpha1}
var T1 = [
...
// LenChange: [z1,z2,pb,beltLen] -> {a,zM,zB,kZ,alpha1}
var LC = [
...
function geo(z1,z2,pb,beltLen){
  ...
  // 精确反解 a: Lp = 2a + C + D/(4a), C=PI/2*(d1+d2), D=(d2-d1)^2
  var C=PI/2*(d1+d2), D=(d2-d1)*(d2-d1);
  // 8a^2 + (2*PI*(d1+d2) - 4Lp)*a + (d2-d1)^2 = 0
  var b=2*PI*(d1+d2)-4*beltLen;
  var a=(-b+Math.sqrt(b*b-32*D))/(8);
  var zM=Math.floor(z1/2 - z1*(d2-d1)/(2*PI*a) + 1e-9);
  var alpha1=180-(d2-d1)/a*57.3;
  var kZ=zM>=6?1:zM===5?0.8:zM===4?0.6:0.4;
  return {a:a,d1:d1,d2:d2,zM:zM,zB:beltLen/pb,kZ:kZ,alpha1:alpha1};
}
function geoA0(z1,z2,pb,a0){
  var d1=z1*pb/PI, d2=z2*pb/PI;
  var cal=2*a0+PI/2*(d1+d2)+(d2-d1)*(d2-d1)/(4*a0);
  return cal;
}
...
T1.forEach(function(c){
  ...
  var pb=PB[bs];
  var d1=z1*pb/PI,d2=z2*pb/PI;
  var cal=geoA0(z1,z2,pb,a0);
  var g=geo(z1,z2,pb,R.beltLen);
  var ok=[],bf=[];
  if(r2(cal)!==R.calBeltLen) bf.push('calBeltLen '+r2(cal)+' vs '+R.calBeltLen); else ok.push('cal');
  if(r3(g.a)!==R.a) bf.push('a '+r3(g.a)+' vs '+R.a); else ok.push('a');
  if(g.zM!==R.zM) bf.push('zM '+g.zM+' vs '+R.zM); else ok.push('zM');
  if(g.zB!==R.zB) bf.push('zB '+g.zB+' vs '+R.zB); else ok.push('zB');
  if(g.kZ!==R.kZ) bf.push('kZ '+g.kZ+' vs '+R.kZ); else ok.push('kZ');
  if(r2(g.alpha1)!==R.alpha1) bf.push('alpha1 '+r2(g.alpha1)+' vs '+R.alpha1); else ok.push('alpha1');
  if(bf.length){bad++;console.log('BAD '+bs+' '+z1+'/'+z2+' a0='+a0+' :: '+bf.join(' | '));}
  else console.log('OK  '+bs+' '+z1+'/'+z2+' a0='+a0);
});
...
LC.forEach(function(c){
  ...
  else console.log('OK  LC '+z1+'/'+z2+' L='+L);
});
