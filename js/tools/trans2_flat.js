...
(function () {
  ...
  App.registerTool({
    ...
    doc: '按《机械设计手册》表13-1-68 流程复刻 原站 平带设计工具：d1=C·∛(P/n₁) 系数法选标准直径 → d2=i·d1·(1−ε) → 轴间距 a₀∈[1.5, 5](d₁+d₂) → 按传动形式（开口/交叉/半交叉）计算带长 L₀ 与包角 α₁ → 选标准内周长度 Lᵢ → <b>胶帆布平带按表13-1-70 双线性插值取 P0（σ0 线性修正）、尼龙片平带按表13-1-76 取 P0</b> → A=Pd/(P0·Kα·Kβ) → 圆整标准带宽 → 压轴力 Fr=2σ₀A·sin(α₁/2)。所有中间量与原站 API 逐字段一致。',
    inputs: [
    ...
    reference: 'GB/T 524-2007《平型传动带》；《机械设计手册》表13-1-68 普通平带传动的设计计算、表13-1-69 工况系数KA、表13-1-70 胶帆布平带单位截面积传递的基本额定功率P0、表13-1-71 包角修正系数Kα、表13-1-72 传动布置系数Kβ、表13-1-73~76 平带规格与尼龙片复合平带P0；原站工具 原站 平带设计',
