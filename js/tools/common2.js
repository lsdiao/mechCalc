 ...
 *  - plate / shell / mechanism 为 原站 formularCalc 服务端查表计算工具，系数表（如
...
(function () {
  ...
  App.registerTool({
    ...
    doc: '复刻自 原站 <b>结构梁计算器</b>（原站该页为 deflection.app 外链聚合资料页，本实现按经典梁理论内置计算）。矩形截面按 W=b·h²/6、I=b·h³/12 计算；单位统一为 N·mm 系（输入 kN、m，内部换算）。',
    inputs: [
    ...
    reference: '原站 结构梁计算器（deflection.app 外链聚合页）；《材料力学》梁弯曲理论。'
  });
  ...
  App.registerTool({
    ...
    doc: '复刻自 原站 <b>紧固件计算工具</b>（原页为紧固云各子工具的外链聚合资料页，本实现按 <i>ISO/GB</i> 常用公式内置计算）。一般公制螺栓扭矩系数 K≈0.2；钢制螺栓 8.8 级最小破坏扭矩按 T_K≈0.24·σb·d³ 估算。',
    inputs: [
    ...
    reference: '原站 紧固件计算工具（fastencloud 外链聚合页）；GB/T 3098.13、ISO 898。'
  });
  ...
  App.registerTool({
    ...
    doc: '复刻自 原站 <b>材料重量计算</b>。理论重量 = 截面积 × 长度 × 密度。内置 Q235/45钢(7.85)、不锈钢(7.93/7.98)、紫铜(8.9)、黄铜(8.5)、铝(2.7) 等密度表；型材取无圆角理论截面近似。',
    inputs: [
    ...
    reference: '原站 计算材料重量；GB/T 702、GB/T 9787 理论重量表。'
  });
  ...
  App.registerTool({
    ...
    doc: '复刻自 原站 <b>矩形平板弯曲计算</b>（6 种支承与载荷工况）。系数表取自《机械设计手册》标准平板计算系数表，按 a/b（或 b/a）比值<b>线性插值</b>，与原站服务端查表一致。挠度单位米(m)、应力单位帕(Pa)，输入按 mm 换算。',
    inputs: [
    ...
    reference: '原站 矩形平板弯曲计算；《机械设计手册》平板计算系数表。'
  });
  ...
  App.registerTool({
    ...
    doc: '复刻自 原站 <b>薄壳中应力与位移计算</b>。采用<b>无矩理论</b>（薄膜理论），仅适用于薄壳（h≪R）无弯曲约束；公式与本页资料一致：球壳 σm=σt=pR/(2h)，圆柱壳环向 σt=pR/h、经向 σm=pR/(2h)。g 取 9.8 m/s²。',
    inputs: [
    ...
    reference: '原站 薄壳中应力与位移计算；《机械设计手册》压力容器薄膜应力公式。'
  });
  ...
  App.registerTool({
    ...
    doc: '复刻自 原站 <b>平面机构的受力分析-构件惯性力的计算</b>。按达朗贝尔原理：惯性力 Pg=-m·ac，惯性力矩 Mg=-Jc·ε；可进一步将惯性力与惯性力矩合成为一个偏离重心 h 的总惯性力（K 点为撞击中心）。',
    inputs: [
    ...
    reference: '原站 平面机构的受力分析-构件惯性力的计算；《机械原理》达朗贝尔原理。'
  });
})();