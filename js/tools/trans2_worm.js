/* =========================================================
 * 蜗杆传动设计计算（1:1 复刻 原站 普通圆柱蜗杆传动设计工具）
...
(function () {
  ...
  App.registerTool({
    ...
    doc: '按 原站「普通圆柱蜗杆传动设计」流程计算：初算传动效率 η₀=(100−3.5√i)/100 与蜗轮转矩 T₂ → 载荷系数 K=K<sub>A</sub>K<sub>V</sub>K<sub>β</sub> → 寿命系数 K<sub>HN</sub> 与许用接触应力 [σ<sub>H</sub>] → <b>按接触强度求最小中心距 a=∛(KT₂·10³(Z<sub>E</sub>Z<sub>ρ</sub>)²/[σ<sub>H</sub>]²)</b> → 选标准中心距 a、模数 m 与 d₁（GB/T 10085 匹配表）→ 几何参数 q、d₂、γ、x₂、m²d₁ → 滑动速度 v<sub>S</sub>、当量摩擦角 φ<sub>V</sub> 与效率 η → 弯曲强度校核 σ<sub>F</sub>=1.53KT₂·10³Y<sub>Fa2</sub>Y<sub>β</sub>/(m³qz₂) → 蜗杆刚度校核与热平衡。所有中间量与原站 API 逐字段一致。',
    inputs: [
    ...
    reference: 'GB/T 10085《圆柱蜗杆传动基本参数》；《机械设计手册》普通圆柱蜗杆传动设计方法（m 与 d₁ 匹配表、使用系数 KA/动载系数 KV/齿向载荷分布系数 Kβ、当量摩擦角 φV 表）；原站工具 原站 普通圆柱蜗杆传动设计。<br><br>' +
