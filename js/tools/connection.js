 ...
 * 6. 平键连接强度校核
 * 7. 压缩弹簧设计计算
 * 依据：《机械设计》（濮良贵版）及相关国家标准，1:1 复刻 原站 布局与计算方式
 * ========================================================= */
(function () {
  'use strict';
  ...
  // 性能等级 → 屈服强度 σs MPa、抗拉强度 σB MPa（与 原站 数据 1:1 一致）
  var GRADE_SS = {
  ...
  App.registerTool({
    ...
    inputs: [
      ...
      { key: 'S', label: '安全系数 S', group: '载荷与材料', type: 'number', default: 1.4, step: 'any', hint: '松螺栓常取 1.2~1.7（与 原站 默认一致）' },
  ...
  App.registerTool({
    ...
    compute: function (v) {
      ...
      var d1 = THREAD_D1[v.d];
      if (!d1) return { error: '未找到所选螺栓的小径数据' };
      var d0 = +v.d0 || +v.d;        // 受剪直径，优先取用户输入，默认取公称直径（与 原站 一致）
  ...
  // 原站 设计推荐序列（不含 M14/M18/M22/M27/M33/M39/M45）
  ...
  App.registerTool({
    ...
    doc: '用于<b>受轴向工作载荷的紧螺栓连接（动载荷）</b>疲劳强度校核与设计。计算螺栓应力幅 σ<sub>a</sub>=λF/(2A)，许用应力幅 [σ<sub>a</sub>]=ε·K<sub>t</sub>·K<sub>u</sub>·σ<sub>-1t</sub>/(K<sub>σ</sub>·S<sub>a1</sub>)，与 原站 计算方式 1:1 一致。',
    inputs: [
      { key: 'mode', label: '计算模式', group: '计算模式', type: 'segment', options: [
        { v: 'check', t: '校核计算' }, { v: 'design', t: '设计计算' }
      ] },
    ...
    reference: 'GB/T 3098.1《紧固件机械性能》；《机械设计》第九版 第四章；原站 螺栓连接（动载荷）。'
  });
  ...
  /* =====================================================
   * 键连接系列 —— 1:1 复刻 原站（calculation_keyconnection 等 6 页）
   * ===================================================== */
  // 平键/楔键 键截面系列（b×h）
  ...
  // 按轴径推荐 平键/楔键 截面与长度（原站 calculation_keysize 实测）
  var FLAT_REC = [
  ...
  // 平键/半圆键 许用挤压应力 [σp]（原站 自动推荐，MPa；实测其 JS：钢 135/101/68、铸铁 75/56/38）
  var SIGP_FLAT = {
  ...
  App.registerTool({
    ...
    brief: '平键连接（静连接/动连接）强度校核，σp=2T/(dkl)，与 原站 1:1 一致。',
    doc: '平键连接强度校核：σ<sub>p</sub> = 2T/(d·k·l) ≤ [σ<sub>p</sub>]，接触高度 k≈0.4h；A 型有效长度 l=L−b、B 型 l=L、C 型 l=L−b/2；双键按 1.5 倍承载。静/动连接仅许用应力不同，与 原站 完全一致。',
    inputs: [
    ...
    reference: 'GB/T 1095-2003、GB/T 1096-2003；原站 平键连接校核计算。'
  });
  ...
  // 按轴径推荐半圆键（原站 calculation_keysize 实测）
  var HALF_REC_LOAD = [
  ...
  App.registerTool({
    ...
    brief: '半圆键连接强度校核：σp=2T/(dkl)，k、l 取自键规格表，与 原站 1:1 一致。',
    doc: '半圆键（GB/T 1098/1099）连接强度校核：σ<sub>p</sub> = 2T/(d·k·L) ≤ [σ<sub>p</sub>]。键规格 b×h×D×L×k 中已含接触高度 k 与长度 L；双键（同一直径上 180°）按 1.5 倍承载。与 原站《半圆键连接校核计算》一致。',
    inputs: [
    ...
    compute: function (v) {
      ...
      return {
        ...
        notes: ['半圆键能在轴槽中摆动自位，装配方便；k、L 直接取自规格表，与 原站 一致。']
      };
    },
    formulas: [
    ...
    reference: 'GB/T 1098-2003、GB/T 1099.1-2003；原站 半圆键连接校核计算。'
  });
  ...
  App.registerTool({
    ...
    brief: '普通楔键连接强度校核：σp=12T/(bl(b+6μd))，与 原站 1:1 一致。',
    doc: '普通楔键（1:100 斜度）连接强度校核：σ<sub>p</sub> = 12T/(b·l·(b+6μd)) ≤ [σ<sub>p</sub>]，μ 为键毂摩擦系数（一般 0.14~0.2）。楔键靠上下面楔紧传扭，对中性差，只用于低速回转或传动精度要求不高的场合。与 原站《普通楔键连接校核计算》一致。',
    inputs: [
      { key: 'T', label: '传递转矩 T', group: '工况', type: 'number', unit: 'N·m', default: 840, step: 'any' },
    ...
    compute: function (v) {
      ...
      return {
        ...
        notes: ['与 原站《普通楔键连接校核计算》1:1 一致：σp = 12T/(b·l·(b+6μd))。']
      };
    },
    formulas: [
    ...
    reference: 'GB/T 1563-2017《楔键》；原站 普通楔键连接校核计算。'
  });

  /* ============ 6d. 切向键连接强度校核 ============ */
  // 按轴径推荐切向键（原站 calculation_keysize 实测：t 厚度、c 倒角、b 宽度、L0=1.5d）
  var TAN_REC = [
  ...
  App.registerTool({
    ...
    brief: '切向键连接强度校核：σp=2T/(d(t−c)l(0.9+μ))，与 原站 1:1 一致。',
    doc: '切向键连接强度校核：σ<sub>p</sub> = 2T/(d·(t−c)·l·(0.9+μ)) ≤ [σ<sub>p</sub>]。切向键由一对 1:100 楔键组成，沿切向楔紧，能传递大转矩，键工作长度常取 l=1.5d；单向传动用一对，双向用两对互成 120°~135°。与 原站《切向键连接校核计算》一致。',
    inputs: [
      { key: 'T', label: '传递转矩 T', group: '工况', type: 'number', unit: 'N·m', default: 840, step: 'any' },
      { key: 'd', label: '轴的直径 d', group: '工况', type: 'number', unit: 'mm', default: 60, step: 'any' },
      { key: 'material', label: '最弱的材料', group: '工况', type: 'select', options: [
    ...
    compute: function (v) {
      ...
      return {
        ...
        notes: ['与 原站《切向键连接校核计算》1:1 一致：σp = 2T/(d·(t−c)·l·(0.9+μ))。']
      };
    },
    formulas: [
    ...
    reference: 'GB/T 1974-2003《切向键》；原站 切向键连接校核计算。'
  });
  ...
  App.registerTool({
    ...
    brief: '矩形花键连接（静/动连接）强度校核：p=2T/(φ·N·h·dm·L)，与 原站 1:1 一致。',
    doc: '矩形花键连接（GB/T 1144）强度校核：p = 2T/(φ·N·h·d<sub>m</sub>·L) ≤ [p]，d<sub>m</sub>=(D+d)/2 为平均直径，h=(D−d)/2−2c 为键齿工作高度，φ 为载荷分布不均系数（一般 0.7~0.8）。[p] 按使用/制造情况与齿面热处理自动推荐。与 原站《矩形花键连接校核计算》一致。',
    inputs: [
      { key: 'connType', label: '连接类型', group: '工况', type: 'segment', options: [
    ...
    reference: 'GB/T 1144-2001《矩形花键尺寸》；原站 矩形花键连接校核计算。'
  });
  ...
  App.registerTool({
    ...
    brief: '渐开线花键连接（静/动连接）强度校核：p=2T/(φ·z·h·d·L)，与 原站 1:1 一致。',
    doc: '渐开线花键连接（GB/T 3478）强度校核：p = 2T/(φ·z·h·d·L) ≤ [p]，d=m·z 为分度圆直径，工作高度 h：30°压力角取 m、45° 取 0.8m，φ 为载荷不均系数（0.7~0.8）。与 原站《渐开线花键连接校核计算》一致。',
    inputs: [
      { key: 'connType', label: '连接类型', group: '工况', type: 'segment', options: [
        { v: 'static', t: '静连接' }, { v: 'dynamic', t: '动连接' }
      ] },
    ...
    reference: 'GB/T 3478.1-2008《圆柱直齿渐开线花键》；原站 渐开线花键连接校核计算。'
  });
